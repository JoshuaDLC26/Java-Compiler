...
// =================================================================================================================================
	...
	// <var list>
	List<VariableDeclaration> variables = parseVariableDeclarationList();
     ...
     */
    private List<VariableDeclaration> parseVariableDeclarationList() {
	...
	return new VariableDeclaration(current, type);
	...
	List<VariableDeclaration> params = parseParameterList(); //method to be implemented
	List<VariableDeclaration> locals = parseVariableList();

	if(proc_token._type == Token.Type.EXTERN) return new ExternalProcedure(name, type, params, locals);
	else if(proc_token._type != Token.Type.PROC) Utility.Error("Expected procedure identifier", proc_token._position);
     ...
     *   <print stmt> ::= 'print(' <expression list> ')'
     *
     * @return A print statement if one is parsed; <code>null</code> otherwise.
     */
	...
	// 'print'
	consumeWhitespace();
	Token printToken = null;
	if ( endOfTokens() || (printToken = _tokens.get(_index++))._type != Token.Type.PRINT ) {
	    _index = startIndex;
	    return null;
	}
	...
	if ( endOfTokens() || (openToken = _tokens.get(_index++))._type != Token.Type.OPENPAREN ) {
	    Utility.error("Open parenthesis expected after print", openToken._position);
	}

	// <expression>
	List<Expression> expressions = parseExpressionList();
	if (expressions == null) {
	    Utility.error("Expressions required for print", openToken._position);
	}
	...
	if ( endOfTokens() || (closeToken = _tokens.get(_index++))._type != Token.Type.CLOSEPAREN ) {
	    Utility.error("Close parenthesis expected at end of print", closeToken._position);
	}

	return new PrintStatement(printToken._position, expressions);	
     ...
     *   <memory> ::= 'memory(' <exp> ')'
     *
     * @return A memory allocation expression, if parsed; <code>null</code> otherwise.
     */
    private MemoryAllocation parseMemory () {
	...
	// 'memory'
	consumeWhitespace();
	Token memoryToken = null;
	if ( endOfTokens() || (memoryToken = _tokens.get(_index++))._type != Token.Type.MEMORY ) {
	    _index = startIndex;
	    return null;
	}
	...
	Token openToken = null;
	if ( endOfTokens() || (openToken = _tokens.get(_index++))._type != Token.Type.OPENPAREN ) {
	    Utility.error("Open parenthesis expected after memory", memoryToken._position);
	}
	...
	if (sizeExpression == null) {
	    Utility.error("Expression required after memory", openToken._position);
	}
	...
	Token closeToken = null;
	if ( endOfTokens() || (closeToken = _tokens.get(_index++))._type != Token.Type.CLOSEPAREN ) {
	    Utility.error("Close parenthesis expected at end of memory", sizeExpression._position);
	}

	return new MemoryAllocation(memoryToken._position, sizeExpression);
     ...
     *
     * @return a Type object that represents the variable type parsed.
     */
	...
	parser._index = 0;
	System.out.println(parser.parseVariableDeclarationList());
    }
